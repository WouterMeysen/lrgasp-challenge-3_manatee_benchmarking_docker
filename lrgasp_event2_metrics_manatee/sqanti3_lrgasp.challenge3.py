...
FIELDS_JUNC = ['isoform', 'chrom', 'strand', 'junction_number', 'genomic_start_coord', 'genomic_end_coord',
               'transcript_coord', 'junction_category', 'start_site_category', 'end_site_category',
               'diff_to_Ref_start_site', 'diff_to_Ref_end_site', 'bite_junction', 'splice_site', 'canonical',
               'RTS_junction', 'indel_near_junct', 'phyloP_start', 'phyloP_end', 'sample_with_cov',
               "total_coverage"]  # +coverage_header

FIELDS_CLASS = ['isoform', 'chrom', 'strand', 'length', 'exons', 'structural_category', 'associated_gene',
                'associated_transcript', 'ref_length', 'ref_exons', 'diff_to_TSS', 'diff_to_TTS', 'diff_to_gene_TSS',
                'diff_to_gene_TTS', 'subcategory', 'RTS_stage', 'all_canonical', 'min_sample_cov', 'min_cov',
                'min_cov_pos', 'sd_cov', 'FL', 'n_indels', 'n_indels_junc', 'bite', 'iso_exp', 'gene_exp', 'ratio_exp',
                'FSM_class', 'coding', 'ORF_length', 'CDS_length', 'CDS_start', 'CDS_end', 'CDS_genomic_start',
                'CDS_genomic_end', 'predicted_NMD', 'perc_A_downstream_TTS', 'seq_A_downstream_TTS',
                'dist_to_cage_peak', 'within_cage_peak', 'pos_cage_peak', 'dist_to_polya_site', 'within_polya_site',
                'polyA_motif', 'polyA_dist', 'ORF_seq', 'TSS_genomic_coord', 'TTS_genomic_coord', 'experiment_id',
                'entry_id']
...
class genePredRecord(object):
    ...
    def from_line(cls, line):
        ...
        return cls(id=raw[0], chrom=raw[1], strand=raw[2], txStart=int(raw[3]), txEnd=int(raw[4]), cdsStart=int(raw[5]),
                   cdsEnd=int(raw[6]), exonCount=int(raw[7]), exonStarts=[int(x) for x in raw[8][:-1].split(',')],
...
class myQueryTranscripts:
    def __init__(self, id, tss_diff, tts_diff, num_exons, length, str_class, subtype=None, genes=None, transcripts=None,
                 chrom=None, strand=None, bite="NA", RT_switching="????", canonical="NA", min_cov="NA",
                 min_cov_pos="NA", min_samp_cov="NA", sd="NA", FL="NA", FL_dict={}, nIndels="NA", nIndelsJunc="NA",
                 proteinID=None, ORFlen="NA", CDS_start="NA", CDS_end="NA", CDS_genomic_start="NA",
                 CDS_genomic_end="NA", ORFseq="NA", is_NMD="NA", isoExp="NA", geneExp="NA", coding="non_coding",
                 refLen="NA", refExons="NA", refStart="NA", refEnd="NA", q_splicesite_hit=0, q_exon_overlap=0,
                 FSM_class=None, percAdownTTS=None, seqAdownTTS=None, dist_cage='NA', within_cage='NA',
                 pos_cage_peak='NA', dist_polya_site='NA', within_polya_site='NA', polyA_motif='NA', polyA_dist='NA',
                 TSS_genomic_coord='NA', TTS_genomic_coord='NA', experiment_id='NA', entry_id='NA'):
    ...
    def __str__(self):
        return "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (
            self.chrom, self.strand, str(self.length), str(self.num_exons), str(self.str_class),
            "_".join(set(self.genes)), self.id, str(self.refLen), str(self.refExons), str(self.tss_diff),
            str(self.tts_diff), self.subtype, self.RT_switching, self.canonical, str(self.min_samp_cov),
            str(self.min_cov), str(self.min_cov_pos), str(self.sd), str(self.FL), str(self.nIndels),
            str(self.nIndelsJunc), self.bite, str(self.isoExp), str(self.geneExp), str(self.ratioExp()), self.FSM_class,
            self.coding, str(self.ORFlen), str(self.CDSlen()), str(self.CDS_start), str(self.CDS_end),
            str(self.CDS_genomic_start), str(self.CDS_genomic_end), str(self.is_NMD), str(self.percAdownTTS),
            str(self.seqAdownTTS), str(self.dist_cage), str(self.within_cage), str(self.pos_cage_peak),
            str(self.dist_polya_site), str(self.within_polya_site), str(self.polyA_motif), str(self.polyA_dist),
            str(self.TSS_genomic_coord), str(self.TTS_genomic_coord),

            str(self.experiment_id), str(self.entry_id))

    def as_dict(self):
        d = {'isoform': self.id, 'chrom': self.chrom, 'strand': self.strand, 'length': self.length,
             'exons': self.num_exons, 'structural_category': self.str_class,
             'associated_gene': "_".join(set(self.genes)), 'associated_transcript': "_".join(set(self.transcripts)),
             'ref_length': self.refLen, 'ref_exons': self.refExons, 'diff_to_TSS': self.tss_diff,
             'diff_to_TTS': self.tts_diff, 'diff_to_gene_TSS': self.tss_gene_diff,
             'diff_to_gene_TTS': self.tts_gene_diff, 'subcategory': self.subtype, 'RTS_stage': self.RT_switching,
             'all_canonical': self.canonical, 'min_sample_cov': self.min_samp_cov, 'min_cov': self.min_cov,
             'min_cov_pos': self.min_cov_pos, 'sd_cov': self.sd, 'FL': self.FL, 'n_indels': self.nIndels,
             'n_indels_junc': self.nIndelsJunc, 'bite': self.bite, 'iso_exp': self.isoExp, 'gene_exp': self.geneExp,
             'ratio_exp': self.ratioExp(), 'FSM_class': self.FSM_class, 'coding': self.coding,
             'ORF_length': self.ORFlen, 'ORF_seq': self.ORFseq, 'CDS_length': self.CDSlen(),
             'CDS_start': self.CDS_start, 'CDS_end': self.CDS_end, 'CDS_genomic_start': self.CDS_genomic_start,
             'CDS_genomic_end': self.CDS_genomic_end, 'predicted_NMD': self.is_NMD,
             'perc_A_downstream_TTS': self.percAdownTTS, 'seq_A_downstream_TTS': self.seqAdownTTS,
             'dist_to_cage_peak': self.dist_cage, 'within_cage_peak': self.within_cage,
             'pos_cage_peak': self.pos_cage_peak, 'dist_to_polya_site': self.dist_polya_site,
             'within_polya_site': self.within_polya_site, 'polyA_motif': self.polyA_motif,
             'polyA_dist': self.polyA_dist, 'TSS_genomic_coord': self.TSS_genomic_coord,
             'TTS_genomic_coord': self.TTS_genomic_coord, 'experiment_id': self.experiment_id,
...
def run_BUSCO(correctedFASTA, cpus, output_dir, ref_dir):
    ...
    if os.path.exists(summary_file):
        ...
        subprocess.call(
            ['busco', '-i', str(correctedFASTA), '-q', '-l', ref_dir + "/busco_data/lineages/eutheria_odb10/",
             '--out_path', output_dir, '--offline', '--download_path', ref_dir + "/busco_data/", '-o', 'BUSCO_analysis',
             '-m', 'transcriptome', '-c', str(cpus)])
        ...
        if not args.gtf:
            if os.path.exists(corrSAM):
                ...
                if args.aligner_choice == "gmap":
                    ...
                    cmd = GMAP_CMD.format(cpus=n_cpu, dir=os.path.dirname(args.gmap_index),
                                          name=os.path.basename(args.gmap_index), sense=args.sense, i=args.isoforms,
                                          o=corrSAM)
                elif args.aligner_choice == "minimap2":
                    print("****Aligning reads with Minimap2...", file=sys.stdout)
                    cmd = MINIMAP2_CMD.format(cpus=n_cpu, sense=args.sense, g=args.genome, i=args.isoforms, o=corrSAM)
                elif args.aligner_choice == "deSALT":
                    print("****Aligning reads with deSALT...", file=sys.stdout)
                    cmd = DESALT_CMD.format(cpus=n_cpu, dir=args.gmap_index, i=args.isoforms, o=corrSAM)
...
def isoforms_parser(args):
    ...
    # gtf to genePred
    cmd = GTF2GENEPRED_PROG + " {0} {1} -genePredExt -allErrors -ignoreGroupsWithoutExons".format(corrGTF, queryFile)
    if subprocess.check_call(cmd, shell=True) != 0:
        print("ERROR running cmd: {0}".format(cmd), file=sys.stderr)
        sys.exit(-1)
...
def transcriptsKnownSpliceSites(refs_1exon_by_chr, refs_exons_by_chr, start_ends_by_gene, trec, genome_dict, nPolyA):
    ...
    isoform_hit = myQueryTranscripts(id=trec.id, tts_diff="NA", tss_diff="NA", num_exons=trec.exonCount,
                                     length=trec.length, str_class="", chrom=trec.chrom, strand=trec.strand, \
                                     subtype="no_subcategory", percAdownTTS=str(percA), seqAdownTTS=seq_downTTS)
                ...
                if ref.exonCount == 1:  # mono-exonic reference, handle specially here
                    if calc_exon_overlap(trec.exons, ref.exons) > 0 and cat_ranking[isoform_hit.str_class] < \
                        ...
                        isoform_hit = myQueryTranscripts(trec.id, "NA", "NA", trec.exonCount, trec.length,
                                                         "geneOverlap", subtype="mono-exon", chrom=trec.chrom,
                                                         strand=trec.strand, genes=[ref.gene], transcripts=[ref.id],
                                                         refLen=ref.length, refExons=ref.exonCount,
                                                         refStart=ref.txStart, refEnd=ref.txEnd, q_splicesite_hit=0,
                                                         q_exon_overlap=calc_exon_overlap(trec.exons, ref.exons),
                                                         percAdownTTS=str(percA), seqAdownTTS=seq_downTTS)
                    ...
                    if match_type == "exact":
                        ...
                        if cat_ranking[isoform_hit.str_class] < cat_ranking["full-splice_match"] or abs(diff_tss) + abs(
                            diff_tts) < isoform_hit.get_total_diff():
                            isoform_hit = myQueryTranscripts(trec.id, diff_tss, diff_tts, trec.exonCount, trec.length,
                                                             str_class="full-splice_match", subtype=subtype,
                                                             chrom=trec.chrom, strand=trec.strand, genes=[ref.gene],
                                                             transcripts=[ref.id], refLen=ref.length,
                                                             refExons=ref.exonCount, refStart=ref.txStart,
                                                             refEnd=ref.txEnd,
                    ...
                    # #######################################################
                        ...
                        if cat_ranking[isoform_hit.str_class] < cat_ranking["incomplete-splice_match"] or (
                            ...
                            diff_tts) < isoform_hit.get_total_diff()):
                            isoform_hit = myQueryTranscripts(trec.id, diff_tss, diff_tts, trec.exonCount, trec.length,
                                                             str_class="incomplete-splice_match", subtype=subtype,
                                                             chrom=trec.chrom, strand=trec.strand, genes=[ref.gene],
                                                             transcripts=[ref.id], refLen=ref.length,
                                                             refExons=ref.exonCount, refStart=ref.txStart,
                                                             refEnd=ref.txEnd,
                    ...
                    # #######################################################
                        ...
                        if cat_ranking[isoform_hit.str_class] < cat_ranking["anyKnownJunction"] or (
                            ...
                            isoform_hit = myQueryTranscripts(trec.id, "NA", "NA", trec.exonCount, trec.length,
                                                             str_class="anyKnownJunction", subtype="no_subcategory",
                                                             chrom=trec.chrom, strand=trec.strand, genes=[ref.gene],
                                                             transcripts=["novel"], refLen=ref.length,
                                                             refExons=ref.exonCount, refStart=ref.txStart,
                                                             refEnd=ref.txEnd,
                                                             q_splicesite_hit=calc_splicesite_agreement(trec.exons,
                                                                                                        ref.exons),
                        ...
                        if cat_ranking[isoform_hit.str_class] < cat_ranking[
                            ...
                            isoform_hit = myQueryTranscripts(trec.id, "NA", "NA", trec.exonCount, trec.length,
                                                             str_class="anyKnownSpliceSite", subtype="no_subcategory",
                                                             chrom=trec.chrom, strand=trec.strand, genes=[ref.gene],
                                                             transcripts=["novel"], refLen=ref.length,
                                                             refExons=ref.exonCount, refStart=ref.txStart,
                                                             refEnd=ref.txEnd,
                        ...
                        if isoform_hit.str_class == "":  # still not hit yet, check exonic overlap
                            if cat_ranking[isoform_hit.str_class] < cat_ranking["geneOverlap"] and calc_exon_overlap(
                                ...
                                isoform_hit = myQueryTranscripts(trec.id, "NA", "NA", trec.exonCount, trec.length,
                                                                 str_class="geneOverlap", subtype="no_subcategory",
                                                                 chrom=trec.chrom, strand=trec.strand, genes=[ref.gene],
                                                                 transcripts=["novel"], refLen=ref.length,
                                                                 refExons=ref.exonCount, refStart=ref.txStart,
                                                                 refEnd=ref.txEnd,
        ...
        if trec.chrom in refs_1exon_by_chr:
            for ref in refs_1exon_by_chr[trec.chrom].find(trec.txStart, trec.txEnd):
                ...
                if isoform_hit.str_class == "":  # no match so far
                    isoform_hit = myQueryTranscripts(trec.id, diff_tss, diff_tts, trec.exonCount, trec.length,
                                                     "full-splice_match", subtype="mono-exon", chrom=trec.chrom,
                                                     strand=trec.strand, genes=[ref.gene], transcripts=[ref.id],
                                                     refLen=ref.length, refExons=ref.exonCount, percAdownTTS=str(percA),
...
def associationOverlapping(isoforms_hit, trec, junctions_by_chr):
    ...
    if len(isoforms_hit.genes) == 0:
        ...
        isoforms_hit.str_class = "genic"  # overlaps with one or more genes on the same strand  # if trec.exonCount >= 2:  #    # multi-exon and has a same strand gene hit, must be NNC  #    isoforms_hit.str_class = "novel_not_in_catalog"  #    isoforms_hit.subtype = "at_least_one_novel_splicesite"  # else:  #    # single exon, must be genic  #    isoforms_hit.str_class = "genic"
...
def write_junctionInfo(trec, junctions_by_chr, accepted_canonical_sites, indelInfo, genome_dict, fout, covInf=None,
    ...
    for junction_index, (d, a) in enumerate(trec.junctions):
        ...
        if phyloP_reader is not None:
            ...
            phyloP_end = ",".join([str(x) for x in
                                   [phyloP_reader.get_pos(trec.chrom, a - 1), phyloP_reader.get_pos(trec.chrom, a),
                                    phyloP_reader.get_pos(trec.chrom, a + 1)]])

        qj = {'isoform': trec.id, 'junction_number': "junction_" + str(junction_index + 1), "chrom": trec.chrom,
              "strand": trec.strand, "genomic_start_coord": d + 1,  # write out as 1-based start
              ...
              "junction_category": junction_cat, "start_site_category": "known" if min_diff_s == 0 else "novel",
              ...
              "indel_near_junct": indel_near_junction, "phyloP_start": phyloP_start, "phyloP_end": phyloP_end,
    ...
    for chrom, records in isoforms_by_chr.items():
        for rec in records:
            ...
            if isoform_hit.coding == "coding":
                ...
                isoform_hit.CDS_genomic_end = m[min(isoform_hit.CDS_end - 1,
                                                    max(m))] + 1  # make it 1-based  # orfDict[rec.id].cds_genomic_start = m[orfDict[rec.id].cds_start-1] + 1  # make it 1-based  # orfDict[rec.id].cds_genomic_end   = m[orfDict[rec.id].cds_end-1] + 1    # make it 1-based
...
def run(args):
    ...
    for r in reader:
        ...
        if (isoforms_info[r['isoform']].canonical == 'NA') or (r['canonical'] == 'non_canonical'):
    ...
    for challenge in challenges_ids:
        ...
        if "manatee_len" in challenge:
            ...
            assessment_sd_len = JSON_templates.write_assessment_dataset(data_id, community, challenge, participant,
                                                                        "sd_len", non_model_results_values[11][0], 0)

            ALL_ASSESSMENTS.extend([assessment_avg_len, assessment_sd_len])

        if "manatee_%_BUSCO_gene_found_vs_complete" in challenge:
            # Calculate the percentage of BUSCO genes found (Complete and single-copy + Complete and duplicated)
            busco_gene_found_percentage = BUSCO_results_values[0][1] + BUSCO_results_values[1][1] + \
                                          BUSCO_results_values[2][1]
            data_id = community + ":" + challenge + "_%BUSCO_gene_found_" + participant
            assessment_busco_gene_found = JSON_templates.write_assessment_dataset(data_id, community, challenge,
                                                                                  participant, "%_BUSCO_gene_found",
                                                                                  busco_gene_found_percentage, 0)
            data_id = community + ":" + challenge + "_%BUSCO_gene_complete_" + participant
            assessment_busco_gene_complete = JSON_templates.write_assessment_dataset(data_id, community, challenge,
                                                                                     participant,
                                                                                     "%_BUSCO_gene_complete",
                                                                                     BUSCO_results_values[0][1] +
                                                                                     BUSCO_results_values[1][1], 0)
            ALL_ASSESSMENTS.extend([assessment_busco_gene_found, assessment_busco_gene_complete])

        if "manatee_%_canonical_SJ_vs_%_SJ_SR_coverage" in challenge:
            data_id = community + ":" + challenge + "_%CanonicalSJ_" + participant
            assessment_canonical_SJ = JSON_templates.write_assessment_dataset(data_id, community, challenge,
                                                                              participant, "%_canonical_SJ",
                                                                              100 - float(
                                                                                  non_model_results_values[9][1]), 0)
            data_id = community + ":" + challenge + "_%SJ_SRCoverage_" + participant
            assessment_SJ_SR_coverage = JSON_templates.write_assessment_dataset(data_id, community, challenge,
                                                                                participant, "%_SJ_SR_coverage",
                                                                                float(non_model_results_values[8][1]),
                                                                                0)
            ALL_ASSESSMENTS.extend([assessment_SJ_SR_coverage, assessment_canonical_SJ])

        if "manatee_trans" in challenge:
            ...
            assessment_num_trans = JSON_templates.write_assessment_dataset(data_id, community, challenge, participant,
                                                                           "num_trans", non_model_results_values[0][0],
                                                                           0)
            ...
            assessment_map_trans = JSON_templates.write_assessment_dataset(data_id, community, challenge, participant,
                                                                           "map_trans", non_model_results_values[1][0],
                                                                           0)
            ALL_ASSESSMENTS.extend([assessment_num_trans, assessment_map_trans])

        if "manatee_num_trans_vs_with_coding_potential" in challenge:
            data_id = community + ":" + challenge + "_num_trans_" + participant
            assessment_num_trans = JSON_templates.write_assessment_dataset(data_id, community, challenge, participant,
                                                                           "num_trans", non_model_results_values[0][0],
                                                                           0)
            data_id = community + ":" + challenge + "_with_coding_potential_" + participant
            assessment_coding_potential = JSON_templates.write_assessment_dataset(data_id, community, challenge,
                                                                                  participant, "with_coding_potential",
                                                                                  non_model_results_values[3][0], 0)
            ALL_ASSESSMENTS.extend([assessment_num_trans, assessment_coding_potential])

        if "manatee_sirvs" in challenge:
            data_id = community + ":" + challenge + "_SIRV_TPR_" + participant
            assessment_TPR = JSON_templates.write_assessment_dataset(data_id, community, challenge, participant, "TPR",
                                                                     SIRVs_results_values[7][0], 0)
        ...
        if 'manatee_%_trans_with_intra-priming_vs_rt_switching' in challenge:
...
def main():
    ...
    parser.add_argument('ref_dir', help='\t Public reference directory including GTF annotation and genome')
    parser.add_argument("challenges_ids", help="\t Space-separated list of challenge IDs to run on")
    ...
    args.genome = args.ref_dir + '/' + "lrgasp_manatee_sirv1.fasta"
